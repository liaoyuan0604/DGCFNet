...
# define the loss
loss = lianheLoss(ignore_index=ignore_index)
#loss = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
use_aux_loss = False

# define the dataloader

train_dataset = PotsdamDataset(data_root='/data/AIfusion/Tony2016Edu/Datasets/Seg_data/data/potsdam/train', mode='train',
                               mosaic_ratio=0.25, transform=train_aug)

val_dataset = PotsdamDataset(transform=val_aug)
test_dataset = PotsdamDataset(data_root='/data/AIfusion/Tony2016Edu/Datasets/Seg_data/data/potsdam/test',
                              transform=val_aug)
