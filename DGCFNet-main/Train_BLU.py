...
from utils.loss import lianheLoss
...
from models.DGCFNet import DGCFNet
from models.A2FPN import A2FPN
from models.MANet import MANet
from models.DCSwin import dcswin_tiny
...
#    'weight_decay': 5e-4,
    ...
    'data_dir': '/data/AIfusion/Tony2016Edu/Yuan_Liao/GeoSeg-main/bludata',
...
def main():
    net = DGCFNet(num_classes=BLU.num_classes + 1).cuda()
    ...
    train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=args['num_workers'],
                              shuffle=True)
    val_set = BLU.Loader(args['data_dir'], 'val', sliding_crop=True, size_context=args['size_context'],
    ...
    criterion = lianheLoss(ignore_index=7).cuda()
#    criterion = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=7).cuda()
    # optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=args['lr'],
    #                       weight_decay=args['weight_decay'], momentum=args['momentum'], nesterov=True)
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, net.parameters()), lr=args['lr'], weight_decay=args['weight_decay'])
    train(train_loader, net, criterion, optimizer, val_loader)
    writer.close()
    print('Training finished.')


def train(train_loader, net, UnetFormerLoss, optimizer, val_loader):
    bestaccT = 0
    bestaccV = 0.5
    bestloss = 1
    curr_epoch = 0
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            main_loss = UnetFormerLoss(outputs, labels)
        ...
        acc_v, loss_v = validate(val_loader, net, UnetFormerLoss, curr_epoch)
...
def validate(val_loader, net, UnetFormerLoss, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            outputs = net(imgs)
            loss = UnetFormerLoss(outputs, labels)
